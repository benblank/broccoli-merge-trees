  ...
  patch.forEach(function(change) {
    ...
    switch(operation) {
      ...
      case 'create':    {
        ...
        return this.out.symlinkToFacadeSync(entry.tree, entry.relativePath, relativePath);
      }
      case 'change':    {
...
BroccoliMergeTrees.prototype._applyMkdir = function (entry, relativePath) {
  if (entry.linkDir) {
    return this.out.symlinkToFacadeSync(entry.tree, relativePath, relativePath);
  } else {
...
BroccoliMergeTrees.prototype._applyChange = function (entry, outputRelativePath) {
  if (isDirectory(entry)) {
    if (entry.linkDir) {
      ...
      this.out.symlinkToFacadeSync(entry.tree, entry.relativePath, outputRelativePath);
    } else {
    ...
    // file changed
    this.out.unlinkSync(outputRelativePath);
    return this.out.symlinkToFacadeSync(entry.tree, entry.relativePath, outputRelativePath);
  }
}
